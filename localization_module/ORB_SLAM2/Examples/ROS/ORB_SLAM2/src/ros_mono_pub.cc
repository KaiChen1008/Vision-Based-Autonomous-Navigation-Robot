...
/**
* see also https://github.com/skylook/ORB_SLAM2-gridmap/ to know the details about how to publish grid map
*
* This modified version allows us to publish poses for the planner module and compute the pose as fast as possible.
* 
*/
...
int main(int argc, char **argv){
	...
	// choose a proper queue size for strikng a balance between performance and the low latency
	// amcl pose
	ros::Publisher 	pub_pose			= nodeHandler.advertise<geometry_msgs::PoseWithCovarianceStamped>("amcl_pose", 1); 

	if (read_from_topic) 
	{
		ImageGrabber igb(SLAM, pub_pts_and_pose, pub_all_kf_and_pts, pub_pose);
		// subscribe topic
		// choose a proper queue size for striing a balance between performance and the low latency
