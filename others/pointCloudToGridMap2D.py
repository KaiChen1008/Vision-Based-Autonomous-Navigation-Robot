...
parser.add_argument('--point_cloud',            dest='point_cloud_fname',           default='results/map_pts_and_keyframes.txt',    help='file name of map_pts_and_keyframes')
parser.add_argument('--keyframe_trajectory',    dest='keyframe_trajectory_fname',   default='results/key_frame_trajectory.txt',     help='file name of key_frame_trajectory')
parser.add_argument('--scale',                  dest='scale_factor',                default=10, type=int,                   help='scale factor')
parser.add_argument('--resize',                 dest='resize_factor',               default=1, type=int,                    help='resize_factor')
parser.add_argument('--filter',                 dest='filter_ground_points',        default=0, type=int,                    help='filter_ground_points')
...
filter_ground_points    = args.filter_ground_points
load_counters           = args.load_counters

point_cloud_fname       = args.point_cloud_fname
keyframe_trajectory_fname = args.keyframe_trajectory_fname
...
if not counters_loaded:
    ...
    for line in point_cloud_data:
        ...
        if filter_ground_points: # default = 0 
            key_frame_id = 0
            ...
            while key_frame_id < len(timestamps):
                try:
                    ...
                    break
                except KeyError: # key is not in dic
                    key_frame_id += 1
...
if output_file_name == None :
    out_fname = 'grid_map_filtered_{:d}_scale_{:d}_resize_{:d}_{:.2f}_{:.2f}'.format(
...
grid_map_resized = np.rot90(grid_map_resized)
